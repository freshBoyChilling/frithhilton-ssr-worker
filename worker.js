...
async function handleRequest(request) {
  const userAgent = request.headers.get('user-agent') || '';
  const isBot = /Googlebot|Bingbot|Slurp/.test(userAgent); // Expand for other crawlers
  const url = new URL(request.url);
  const path = url.pathname;
      ...
      if (response.ok) {
        ...
        const html = `
          ...
          <html lang="en">
            <head>
              <title>${song.song_title} by Frith Hilton</title>
              <meta name="description" content="Lyrics and cover for ${song.song_title} by Frith Hilton from ${album.name}. Released ${song.release_date}.">
            ...
            <body>
              ...
              <p>From album: ${album.name} (${album.date}) | Released: ${song.release_date} | Duration: ${song.duration} seconds</p>
              <img src="${data.cover}" alt="Cover for ${song.song_title} by Frith Hilton" width="300">
              <h2>Lyrics</h2>
              ${lyricsHtml}
              <audio controls><source src="${data.audio}" type="audio/mpeg"></audio>
            </body>
          </html>
        `;
...
d.com/frithhilton17", "https://genius.com/artists/Frith-hilton", "https://instagram.com/frithhilton17"],
                    "contactPoint": {"@type": "ContactPoint", "email": "hello@frithhilton.com.ng", "contactType": "Professional Contact"},
                    "worksFor": {"@type": "Organization", "name": "Frith Nightswan Enterprises", "url": "https://www.frithnightswanenterprises.com.ng"}
                  },
                  "recordingOf": {
